...
const ExamDetail = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/api/getexaminstance/${id}`
        );
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const examInstanceResponse = await axios.post(
        `http://localhost:5000/api/create-exam-instance/${id}`,
        {
          status,
          takenBy,
          createdBy,
      ...
      console.log(
        ...
        examInstanceResponse.data
      );

      // Update exam definition
      const examDefinitionResponse = await axios.post(
      ...
      console.log(
        "Exam Definition Updated Successfully:",
        examDefinitionResponse.data
      );
    } catch (error) {
